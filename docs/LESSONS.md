...
11. **NativeWind v4 full-screen layout bug** — 5 fix attempts over 2 sessions. Root cause: NativeWind's jsxImportSource wraps ALL RN components and can strip layout styles silently. Patches (inline styles, StyleSheet, cssInterop={false} on individual components) each fixed one symptom and revealed the next. The architectural fix: bypass NativeWind entirely on the screen — useSafeAreaInsets() hook, StyleSheet.create for everything, cssInterop={false} on every component, zero className usage. Apply this pattern to any screen with ImageBackground + gradient + pinned bottom content. Debug borders (red/lime/cyan) were the diagnostic that finally identified the layout layer vs style layer distinction.
